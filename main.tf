...
resource "aws_default_security_group" "update" {
  ...
  ingress {
    cidr_blocks = [for subnet in data.aws_subnet.each : subnet.value.cidr_block]
...
module "webapp_aws_lambda_layer_version" {
  ...
  compatible_runtimes      = ["python3.12"]                    # Optional argument, but keep it.
...
module "webapp_aws_lambda_function" {
  ...
  runtime                        = "python3.12"                                  # Optional argument, but keep it.
